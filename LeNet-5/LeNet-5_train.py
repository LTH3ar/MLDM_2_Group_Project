...
for epoch in range(NUM_EPOCHS):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % 10 == 0:  # every 100 mini-batches
            ...
            print(
                f'Train Epoch: {epoch_count} [{batch_idx}/{NUM_BATCHES} ({100. * batch_idx / NUM_BATCHES:.0f}%)]\tAccuracy: {acc.item():.6f}')
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(val_loader):
            ...
            if batch_idx % 10 == 0:  # every 100 mini-batches
                ...
                print(
                    f'Validation Epoch: {epoch_count} [{batch_idx}/{NUM_BATCHES_VAL} ({100. * batch_idx / NUM_BATCHES_VAL:.0f}%)]\tAccuracy: {acc.item():.6f}')
